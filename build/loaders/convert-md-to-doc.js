...
async function resolveDemoTitle(fileName, demoEntryPath) {
  const demoStr = await fse.readFile(path.resolve(projectPath, demoEntryPath, '..', fileName), 'utf-8')
...
async function convertMd2ComponentDocumentation(text, url, env = 'development') {
...
<template>
  ...
  >
    ...
    <div style="width: 144px;" v-if="showAnchor">
      ${demoInfos.length ? genDemosAnchorTemplate(demoInfos) : genPageAnchorTemplate(tokens)}
    </div>
  </div>
</template>`
