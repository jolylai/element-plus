...
import { NConfigProvider, darkTheme, enUS, zhCN, dateEnUS, dateZhCN, useOsTheme } from 'naive-ui'
// import { TsConfigProvider } from '../../themes/tusimple/src'
import { i18n, useIsMobile } from '../utils/composables'
import { createDocumentationMenuOptions, createComponentMenuOptions } from './menu-options'
...
const configProviderRef = computed(() => {
  return configProviderNameRef.value === 'tusimple' ? TsConfigProvider : NConfigProvider
})
...
export function push(partialPath) {
  ...
  router.push(fullPath.replace(/(^\/[^/]+\/[^/]+)((\/.*)|$)/, '$1' + partialPath))
}
